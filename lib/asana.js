...
// Is the Asana CLI actually installed here?
//
// This extension is also run on a client's workstation, which has no Asana CLI and
// no bridge. Everything Asana-shaped — the queue buttons, the task links, the Agent
// Sessions view — hangs off this one answer, so that machine gets a clean sidebar
// instead of dead buttons and a spawn that fails with ENOENT on every refresh.
//
// The setting alone cannot answer it: its default (~/tools/asana/asana) is a
// non-empty string everywhere, so every caller's `if (!cmd) return` guard passes on
// a box where nothing is installed. The file has to be looked at.
let _asanaAvailable;
function asanaAvailable() {
  if (_asanaAvailable !== undefined) return _asanaAvailable;
  _asanaAvailable = false;
  try {
    const bin = expandHome(((cfg().get('asanaCommand') || '').trim().split(/\s+/)[0]) || '');
    if (bin) { fs.accessSync(bin, fs.constants.X_OK); _asanaAvailable = true; }
  } catch {}
  return _asanaAvailable;
}

// Called when the configuration changes; the answer only moves when the setting does.
function forgetAsanaAvailable() { _asanaAvailable = undefined; }

// Empty when the CLI is missing, which is what every caller already treats as
// "do nothing" — so the early returns that were written for an empty setting now
// also cover an absent binary.
function asanaCommand() { return asanaAvailable() ? expandHome((cfg().get('asanaCommand') || '').trim()) : ''; }
...
// The icon leads the session name too, not just the button: a row of tabs reading
// "📅 inbox-zero · Today" says what the session is about before the words are read.
// The joined scope is one session over the union of the two queues, not two sessions:
// they filter on different axes ('today' on assignee + due date, 'input' on the
// Priority field), so a task that is both sits in both result sets — 2 of 3 input
// tasks on 2026-09-04. `asana queue today+input` merges them, so the walkthrough sees
// each task once. Two parallel walkthroughs would also be two conversations competing
// for the same person's answers, which is the thing inbox-zero exists to economise.
const QUEUE_SCOPES = [
  { id: 'today', icon: '\ud83d\udcc5', label: 'Today', prompt: '/inbox-zero today' },
  { id: 'input', icon: '\u23f3', label: 'Input', prompt: '/inbox-zero input' },
  { id: 'today+input', icon: '\ud83d\udce5', label: 'Today + Input', prompt: '/inbox-zero today+input' },
];
...
// Every project a target answers for. A client is often more than its "<CODE> EDV":
// "Audioveröffentlichung klappt nicht" sits in "MP Produktion", which is SFF's work, and
// with only the primary gid to compare against, the exact hit on it matched no target and
// the entry went to the model to be routed on wordplay.
function targetGids(x) {
  return (x && Array.isArray(x.gids) && x.gids.length) ? x.gids : (x && x.gid ? [x.gid] : []);
}
...
function matchAsanaTask(names, list, targets) {
  ...
  for (const t of list) {
    ...
    if (!projects.length) { homeless.push(t); continue; }
    const target = targets.find((x) => targetGids(x).some((g) => projects.some((p) => p && p.gid === g)));
    if (target) resolved.push({ task: t, target });
  }
  // A task in no project is what the phone's share sheet makes: it sits in Pat's My Tasks
  // and says nothing about where it belongs. It is still the task the entry names, so it
  // is returned without a target — the model's routing picks the folder, and the session
  // is told to file it into that project instead of creating a copy of it.
  if (!resolved.length) {
    const open = homeless.find((t) => !t.completed);
    return open ? { target: null, task: { ...open, homeless: true } } : null;
  }
...
function findAsanaTask(question, targets) {
  return new Promise((resolve) => {
    ...
    try {
      child = cp.execFile(
        // As Pat: the AI token cannot see a task that is in no project, and the search
        // leaves it out without an error — which filed a second copy of it, twice.
        tokens[0], [...tokens.slice(1), 'find', query, '--limit', '20', '--json', '--as-pat'],
...
// The bridge's own map of Asana project name → client code (CLIENT_CODE_MAPPING in the
// infra config next to the CLI). It is where a project whose name is not "<CODE> …" —
// "MP Produktion" for SFF, "RA Sven Hörnich" for RAH — is tied to its client, and reading
// it here means the box and the bridge put the same task in the same folder. Empty on a
// box without the CLI, or if the config cannot be loaded.
let _clientCodeMapping;
function clientCodeMapping() {
  if (_clientCodeMapping !== undefined) return _clientCodeMapping;
  _clientCodeMapping = {};
  try {
    const bin = asanaCommand().split(/\s+/)[0];
    if (bin) {
      const m = require(path.join(path.dirname(fs.realpathSync(bin)), 'config.js')).CLIENT_CODE_MAPPING;
      if (m && typeof m === 'object') _clientCodeMapping = m;
    }
  } catch {}
  return _clientCodeMapping;
}
...
module.exports = {
  asanaAvailable, forgetAsanaAvailable, asanaCommand, asanaCacheFile, parseAsanaProjects, loadAsanaProjects, refreshAsanaProjects,
  QUEUE_SCOPES, queueCountsFile, loadQueueCounts, refreshQueueCounts, asanaTaskQuery,
  asanaTaskNames, taskProjects, sameTaskName, targetGids, matchAsanaTask, findAsanaTask, HOUSE_PROJECTS, clientCodeMapping,
