...
// Has this machine ever had bridge sessions? The Agent Sessions view is shown when
// the Asana CLI is here OR one of these files exists, so a box that used to run the
// bridge keeps its history readable after the CLI goes away.
function hasAgentSessionSource() {
  for (const f of [agentIndexFile, agentHistoryFile]) {
    try { if (fs.existsSync(f())) return true; } catch {}
  }
  return false;
}
...
module.exports = {
  hasAgentSessionSource, tmuxHasSession, agentSocket, agentIndexFile, readAgentIndex, writeAgentIndex, tmuxAlive,
