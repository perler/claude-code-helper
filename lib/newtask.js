...
const {
  HOUSE_PROJECTS, QUEUE_SCOPES, asanaAvailable, clientCodeMapping, asanaTaskQuery, findAsanaTask, loadAsanaProjects, loadQueueCounts, refreshAsanaProjects, refreshQueueCounts,
} = require('./asana');
...
// All of a client's projects, not just the one work gets filed in: an existing task is
// recognised by the project it already sits in, whichever of them that is. A project
// belongs to the client when its name leads with the code, or when the bridge's mapping
// names the client for it.
function clientProjects(code, projects) {
  const want = code.toUpperCase();
  const map = clientCodeMapping();
  return projects.filter((p) => !/\bold\b/i.test(p.name)
    && !HOUSE_PROJECTS.some((h) => h.re.test(p.name))
    && (p.name.split(/\s+/)[0].toUpperCase() === want
      || String(map[p.name.trim()] || '').toUpperCase() === want));
}
...
function listTargets() {
  ...
  for (const code of [...new Set(folders.map((f) => f.replace(/^#/, '')))].sort()) {
    ...
    out.push({
      ...
      name: p ? p.name : code,
      gid: p ? p.gid : '',
      gids: [...new Set([...(p ? [p.gid] : []), ...clientProjects(code, projects).map((x) => x.gid)])],
      dir,
  ...
  const used = new Set(out.flatMap((t) => t.gids || [t.gid]).filter(Boolean));
...
// Our own alerts come from our own addresses, so the sender names no client — and left
// to the model, "healthchecks@patsplanet.com" read as hosting. The check names it: every
// monitor is called "<code>-<host>-<job>", so "DOWN | rah-wks004-macrium" is RAH's. Only
// a leading segment that IS a client shortcode counts, so RAH never becomes RAHR.
const ALERT_SENDER = /(^|[@.])(patsplanet\.com|healthchecks\.io)$/i;
function alertClient(mail, targets) {
  if (!mail || mail.client || !ALERT_SENDER.test(String(mail.from_address || ''))) return '';
  for (const m of String(mail.subject || '').matchAll(/(?:^|[^\w-])([a-z0-9]+)-[a-z0-9]/gi)) {
    const code = m[1].toUpperCase();
    if (targets.some((t) => t.id.toUpperCase() === `CLIENT:${code}`)) return code;
  }
  return '';
}
...
async function generateSessionPlan(question, known) {
  ...
  let mail = known || await findMail(question);
  const alertCode = alertClient(mail, targets);
  if (alertCode) mail = { ...mail, client: alertCode };
  ...
  if (hit) { if (hit.target) plan.target = hit.target; plan.task = hit.task; }
...
// A task in no project is invisible to the AI token (403 on read, left out of a search),
// so the session has to file it into a project, as Pat, before it can touch it.
function homelessNote(task, target) {
  if (!task || !task.homeless) return [];
  const into = target && target.gid ? `project "${target.name}" (${target.gid})` : 'the right project';
  return [
    `It is in Pat's My Tasks with no project, so the AI token cannot see it. Add it to ${into} first,`,
    `with Pat's token: POST /tasks/${task.gid}/addProject using ASANA_ACCESS_TOKEN (asana-workflow.md,`,
    '"A project-less task is invisible"). After that, work it as the AI as usual.',
  ];
}
...
async function askClaudeSession(question, io, known) {
  ...
  for (;;) {
    ...
    // having found it.
    existing = null;
    // A task in no project belongs to no target, so rejecting one takes nothing from it.
    if (!(task && task.homeless)) task = null;
...
class AskViewProvider {
  ...
  async _startMail(post) {
    ...
    if (!list.length) {
      vscode.window.showInformationMessage('Nothing in the inbox for us — no client mail, no alerts from our own systems.');
      return;
    }
    const items = [
      {
...
<div id="queues"${asanaAvailable() || mailAvailable() ? '' : ' hidden'}>
  ...
  <button data-mail="1" title="Inbox mail for us — client mail and our own alerts, one of them or all of them"${mailAvailable() ? '' : ' hidden'}>✉️ Mail <span class="n" id="n-mail">–</span></button>
