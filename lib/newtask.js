...
class AskViewProvider {
  ...
  postAsanaState() {
    try { this.view.webview.postMessage({ type: 'asana', available: asanaAvailable() }); } catch {}
  }
...
<div id="queues"${asanaAvailable() ? '' : ' hidden'}>
...
<script nonce="${nonce}">
  ...
  window.addEventListener('message', (e) => {
    const m = e.data || {};
    if (m.type === 'asana') { queues.hidden = !m.available; return; }
    if (m.type === 'counts') {
