...
function rememberTabName(tabId, name) {
  if (!name || !/^[0-9a-f-]{36}$/.test(String(tabId || ''))) return;
...
// Put the recorded name back on every tab that came up under a different one.
//
// What VS Code substitutes depends on how far its cwd detection has got, so there is
// no shape to match on: the same "📅 inbox-zero · Today" tab came back as "📅" after
// one reload and as "work" — terminal.integrated.tabs.title is "${cwdFolder}" here,
// and the session runs in ~ — after the next. So the test is simply "not what we set".
//
// Each terminal is repaired at most ONCE per window, which is what keeps this from
// fighting a rename typed by hand: the sweeps all run in the first half-minute after
// activation, and a name changed after that is left alone for good.
const tabNamesRepaired = new WeakSet();

async function repairTabNames() {
  const all = readTabNames();
  if (!Object.keys(all).length) return;
  for (const terminal of vscode.window.terminals) {
    if (terminal.exitStatus || tabNamesRepaired.has(terminal)) continue;
    let key;
    try { key = tabStateKeyForTerminal(terminal); } catch { key = null; }
    // No key yet means the shell's pid has not been resolved — a later sweep gets it.
    if (!key) continue;
    const want = all[key] && all[key].name;
    if (!want) { tabNamesRepaired.add(terminal); continue; }
    tabNamesRepaired.add(terminal);
    if (want === terminal.name) continue;
...
async function renameTerminalTab(terminal, name) {
  ...
  terminal.show(true);
  // renameWithArg renames whatever terminal is ACTIVE, and show() only *queues* the
  // switch — it does not wait for it. Firing the command straight after it therefore
  // renames whichever tab was still active, which is invisible when one tab is being
  // renamed (it is the one that was just shown) and swaps names between tabs the
  // moment a sweep renames several in a row: on the 06:37 reload of 2026-09-04 the
  // Today session's name landed on the tab next to it and vice versa. So wait for the
  // switch — and if it never comes, do nothing at all: no name beats a wrong name.
  for (let i = 0; i < 40 && vscode.window.activeTerminal !== terminal; i++) {
  ...
  // Recorded only once it actually landed, and only against a launch id — a cwd
  // fallback key belongs to a folder, not to a session, so a later session in the
  // same folder would inherit the name.
